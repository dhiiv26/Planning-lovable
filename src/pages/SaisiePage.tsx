                  ...
                  {SHIFT_CODES.map(s => (
                    <SelectItem key={s.code} value={s.code}>
                      <span className="flex items-center gap-2">
                        <span className={`inline-block w-2 h-2 rounded-full`} style={{ backgroundColor: `hsl(var(--shift-${s.category}))` }} />
                        {s.code} — {s.label} ({s.hours}h, {s.time})
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
                    ...
                    return (
                      <div key={entry.date} className="flex items-center justify-between rounded-md border p-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold border ${getShiftClass(entry.shiftCode)}`}>
                            {entry.shiftCode}
                          </span>
